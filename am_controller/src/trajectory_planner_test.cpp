...
double obstacle_hit_range_links(double x[],double y[])
{
	...
	// 	for(int j=0;j<4;j++)
	// 	{
	// 		obs_gap=sqrt((x[j]-ox)*(x[j]-ox)+(y[j]-oy)*(y[j]-oy));
	// 		if(obs_gap<=obstacle_safe_range)hit_cnt+=1;//((obstacle_safe_range-obs_gap)*(obstacle_safe_range-obs_gap));
	// 	}
	// }
	// return hit_cnt;
	                    ...
	                    double *constr) 
{
	...
	//decode from real variable.
	    ...
	    //  Kin.Forward_Kinematics(thetam_0,thetam_1,thetam_2,global_theta_4);
	    // if(Valid_Angle(thetam_0*180.0/pi,thetam_1*180.0/pi,thetam_2*180.0/pi,global_theta_4*180.0/pi))
	    // {
	    // 	printf("valid_median:%lf %lf %lf xy %lf %lf\n",thetam_0,thetam_1,thetam_2,Kin.t1,Kin.t2);
	    // 	getchar();
	    // }
	 //calculate links ab
			...
			double af_0,double af_1,double af_2)
{
	...
	plot_data.open("plot_data.out" , ios::out | ios::trunc);
	   ...
	   link_chrom links_opt;
	    ...
	    // plot_data<<endl;
	    // Kin.Forward_Kinematics(theta0_0,theta0_1,theta0_2,global_theta_4);
	    // if(Valid_Angle(theta0_0*180.0/pi,theta0_1*180.0/pi,theta0_2*180.0/pi,global_theta_4*180.0/pi)==false)constr=1;
	    // 	else constr=0;
	    // plot_data<<theta0_0<<" "<<theta0_1<<" "<<theta0_2<<" "<<Kin.t1<<" "<<Kin.t2<<" "<<constr<<" "<<t<<endl;
	    //  Kin.Forward_Kinematics(thetaf_0,thetaf_1,thetaf_2,global_theta_4);
	    //  if(Valid_Angle(thetaf_0*180.0/pi,thetaf_1*180.0/pi,thetaf_2*180.0/pi,global_theta_4*180.0/pi)==false)constr=1;
	    // 	else constr=0;
	    // plot_data<<thetaf_0<<" "<<thetaf_1<<" "<<thetaf_2<<" "<<Kin.t1<<" "<<Kin.t2<<" "<<constr<<" "<<t<<endl;
...
void obstacle_generator(double dx,double dy)
{
	...
	for(double ox=dx-0.12;ox<=dx+0.12;ox+=obstacle_point_gap)//for(double ox=dx-0.03;ox<=dx+0.03;ox+=obstacle_point_gap)
	{
		double oz=dy+0.04;
	...
	for(double oz=dy+0.04;oz>=dy-0.04;oz-=obstacle_point_gap)
	{
